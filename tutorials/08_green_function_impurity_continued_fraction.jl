...
println("Initial-parameters :")
@show BathParamReshapePH( BParamDnPH, nbathHalf )
...
@time cost = GetCostFromFlatPH( BParamDnPH, SParam )
println( "initial cost : $(cost) " )

prob = OptimizationProblem(GetCostFromFlatPH, BParamDnPH, SParam)
...
NReFreq = 200
wmax    = abs(U)<1e-5 ? 2 : U
wFreq   = collect(LinRange( -wmax, wmax, 200 ))
wFreqBr = collect(LinRange( -wmax, wmax, 200 )) .+ epsilon * im

ydat1   = GetGreenFromHCFcoeffA(    H.MatSparse, cv1,  wFreqBr, Eimp0 )
ydat2   = GetGreenFromHCFcoeffAdag( H.MatSparse, cdv1, wFreqBr, Eimp0 )
ydat    = ydat1+ydat2

# ydat = zeros(Float64,length(wFreq))
# for ii in 1:nfreq
#     energy  = wFreq[ii]
#     T       = deepcopy(T_orig)
#     dumy    = continued_fraction(T, m, energy + Eimp0 + 0.001 *im)
#     ydat[ii]    = imag(dumy)
#     println( "$(energy) : $(real(dumy))  + im * $(imag(dumy)) " )
# end

yrD2  = GetDeltaHybDiscGrid( enew, vnew, wFreqBr )
yr2  = GetGreenImpGrid( zeros(1,1), yrD2, wFreqBr )
